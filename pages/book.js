...
import { format, addMonths, subMonths, startOfMonth, endOfMonth, eachDayOfInterval, getDay, isSameDay, isToday, parseISO } from 'date-fns'
...
export default function BookPage() {
  ...
  async function submitBooking(e) {
    ...
    const { error } = await createBooking({
      ...
      vehicle, notes, status: 'confirmed'
    })
    setSubmitting(false)
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 2rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <button onClick={() => setCurrentMonth(m => subMonths(m, 1))} style={{ width: 36, height: 36, borderRadius: '50%', border: '1.5px solid var(--border)', background: 'var(--white)', cursor: 'pointer', fontSize: 16 }}>‹</button>
            <h2 style={{ fontSize: 28 }}>{format(currentMonth, 'MMMM yyyy')}</h2>
            <button onClick={() => setCurrentMonth(m => addMonths(m, 1))} style={{ width: 36, height: 36, borderRadius: '50%', border: '1.5px solid var(--border)', background: 'var(--white)', cursor: 'pointer', fontSize: 16 }}>›</button>
          </div>
          <div style={{ display: 'flex', gap: 16, alignItems: 'center', fontSize: 12, color: 'var(--text-muted)', flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 24 }}>
                      ...
                      {Array.from({ length: 7 }).map((_, colIdx) => {
                        ...
                        return (
                          <td key={colIdx}
                            ...
                            {isMine && <div className="cal-event ev-mine">My booking</div>}
                          </td>
                        )
                      })}
                    </tr>
          ...
          <div className="card">
            <div className="card-header">
              <span className="card-title">Available Times</span>
            </div>
            <div className="card-body">
              {!selectedDate ? (
                <div className="empty-state">
                  <div className="empty-icon">📅</div>
                  <p>Click a date to see available times</p>
                </div>
              ) : loadingSlots ? (
                ...
                <>
                  ...
                  <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 16 }}>
                    {TIMES.length - takenTimes.length} slots available
                  </p>
            ...
            <div className="modal-body">
              <form onSubmit={submitBooking}>
                <div className="form-group">
                  ...
                  <input readOnly value={selectedDate ? `${format(selectedDate, 'MMM d, yyyy')} @ ${selectedTime}` : ''} style={{ background: 'var(--gray)' }} />
                </div>
                <div className="form-group">
                ...
                <div className="form-group">
                  ...
                  <input type="text" value={vehicle} onChange={e => setVehicle(e.target.value)} placeholder="e.g. 2018 Ford Mustang GT" required />
                </div>
                <div className="form-group">
                  <label>Notes</label>
